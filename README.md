...
| g++ | C++17（实际 11.4） | 编译 |
| OpenCV | 4.10.0 | 图像/视频处理 + DNN 推理（任务3 直接加载 ONNX） |
| Eigen3 | 3.4.0 | 矩阵运算、最小二乘 |
| Python3 + numpy + matplotlib | 3.10 / 1.26 / 3.5 | 任务 2 出图 |

> 任务 3 的推理用 OpenCV 自带的 `cv::dnn` 模块加载 `model/best.onnx`，
> 无需安装 PyTorch / ultralytics / ONNX Runtime 等推理框架。
> 验证任务 3 只需装 OpenCV + Eigen3（`sudo apt install libopencv-dev libeigen3-dev`）。
...
红色在色相环上跨越 H 两端，用两段区间取并集：`[0,13]` ∪ `[170,179]`。（第一段区间取更多，为了花瓣边缘红黄交界部分的准确框选）
